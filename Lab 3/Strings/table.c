...
*   Description: Progam handles string fucnctions to be called
*/
...
//Set contains all required variables and structures
typedef struct set
{
	int count;
    	int length;
...
//Function allocates mem for and creates sets
//Big-O: O(n)
...
//Functions returns number of elements
//Big-O: O(1) 
...
//Function adds elements by hashing them and inserting
//Big-O: O(n)
...
//Function hashes then removes elements with freeing from memeory
//Big-O: O(n)
...
//Function finds elt and returns the pointer
//Big-O: O(n)
...
//Function allocates and copys char** data then returns
//Big-O: O(n)
...
//Function uses a hash function to serch for elt returning index
//Big-O: O(n)
static int search(SET *sp, char *elt, bool *found)
{
	assert(sp != NULL);
