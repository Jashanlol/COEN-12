...
void change(void *array[], int first, int second);
...
*   Complexity: O(n) 
*
*   Description: The function getElements will return a sorted array copy of data
*/
...
//Recursive function to sort values in the param array
//Big-O: O(n)
...
//Function to find the partition value to use in quicksort
//Big-O: O(n)
int partition(void *array[], int (*compare)() ,int left, int right)
{
...
//Function to compare values and find the median to swap
//Big-O: O(1) 
void *median(void *array[], int (*compare)() ,int left, int right)
{
	int midpoint = (right - left) /2;
    	...
    	{
        	change(array, left, midpoint);
    	}

  	if(compare(array[midpoint], array[right]) < 0)
	{
    		change(array, midpoint, right);
  	}
  	return array[midpoint];
}

//Function swaps two values in an array
//Big-O: O(1)
void change(void *array[], int first, int second)
{
	void *p = NULL;
